# AGENTS.md — Agency Operating System
...
Before starting any meaningful task, Codex must do the following:
...
Codex must not assume important product, business, design, pricing, security, or workflow details without asking.

## Spec Before Build Rule

For any significant build task, Codex must create or confirm a proper spec before implementation.
...
If the task is unclear, Codex should ask questions before building.

## Verification Before Done Rule

Before saying a task is complete, Codex must run the relevant verification.
...
For larger tasks, Codex should use subagents or separate perspectives to improve quality.

Codex should decide the number of subagents based on task complexity.
...
Codex should not use subagents for every small task. Use them when they reduce risk or improve judgment.

## Skills Rule

Before starting a task, Codex must check `SKILLS.md`.
...
If a task is repetitive and likely to happen often, Codex should suggest creating or updating a reusable skill/process and then update `SKILLS.md` only after approval.

## Gotchas Rule

Codex should maintain awareness of repeated mistakes and avoid them.
